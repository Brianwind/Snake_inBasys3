module VGA(input CLK,
    ...
    always @(posedge CLK) begin//绘制
        ...
        if(Vsync_s>=0&&Vsync_s<10&&Hsync_s>=0&&Hsync_s<10) begin
            ...
            else if(map[40*row_block+col]==1) begin
                if (mist==1&&(40*row_block+col)%16>7) begin//迷雾下为黑色
                    RED<=4'b0000;
                    GRN<=4'b0000;
                    BLU<=4'b0000;
                end
                else if(row_block==head1_y&&col==head1_x) begin//头是青色
                    RED<=4'b0000;
                    GRN<=4'b1111;
                    BLU<=4'b1111;
                end
            ...
            if(Hsync_s>=1748&&Hsync_s<1856) begin
                score<=score_m;
                y<=y_m;
                x<=x_m;
                if(cnt_m==3) begin
            ...
            else if(Hsync_s>=1856&&Hsync_s<1964) begin
                score<=score_l;
                y<=y_l;
                x<=x_l;
                if(cnt_l==3) begin
